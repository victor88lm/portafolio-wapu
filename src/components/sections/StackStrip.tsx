...
function StackItems({ hidden = false }: { hidden?: boolean }) {
  return (
    <ul
      className="flex shrink-0 items-center gap-x-3 pr-3 sm:flex-wrap sm:items-baseline sm:gap-y-1.5 sm:pr-0"
 ...
 * vuelve al readout estático del sistema.
 */
export function StackStrip() {
  return (
    <div className="overflow-hidden border-y border-line bg-white/60">
      <Container className="py-4 font-mono text-[12px] sm:flex sm:flex-wrap sm:items-baseline sm:gap-x-3 sm:gap-y-1.5">
        <span aria-hidden="true" className="hidden shrink-0 font-semibold text-brand-700 sm:inline">
          $ stack
        </span>
        <div className="-mx-5 overflow-hidden whitespace-nowrap sm:mx-0 sm:overflow-visible sm:whitespace-normal">
          <div className="stack-marquee flex w-max items-center sm:w-auto sm:animate-none sm:flex-wrap">
            <StackItems />
